...
удаление `password_hash`).

## Регистрация приложения VK ID (для входа через VK, Шаг 25)

Ручной one-time шаг в кабинете id.vk.com — аналогично уже описанному
ручному добавлению сообщества в VK-беседу (см. ARCHITECTURE.md,
«Уведомления водителям»). Без этого шага код Шага 25 задеплоен и не
падает, но кнопка «Войти через VK» ни для кого не будет работать —
`VK_CLIENT_ID`/`VK_CLIENT_SECRET` пустые.

1. Завести приложение в кабинете id.vk.com (нужен VK-аккаунт
   администратора сообщества/группы).
2. Указать `redirect_uri`: `https://zakaz.glorden.ru/auth/vk/callback`.
3. **Перед тем как писать/деплоить финальный код** — сверить в
   документации id.vk.com точные значения, помеченные `# СВЕРИТЬ` в
   `app/vk_oauth.py` (URL авторизации/обмена токена/user info, форма
   ответа, где передаётся `client_secret`) — на момент реализации Шага 25
   официальная документация была недоступна для прямой проверки, в коде
   остались рабочие гипотезы.
4. Добавить в прод-`.env` (рядом с уже существующими переменными,
   `VK_GROUP_TOKEN`/`VK_PEER_ID`/`SITE_URL` — это другая, не связанная
   пара):
   ```
   VK_CLIENT_ID=<из кабинета id.vk.com>
   VK_CLIENT_SECRET=<из кабинета id.vk.com>
...
Технический долг с identity-проверками в `/orders/{id}/respond`,
`/orders/{id}/self-assign`, `/orders/{id}/complete`,
`/orders/{id}/cancel` и `POST/PUT/DELETE /price`
(см. [ARCHITECTURE.md](./ARCHITECTURE.md)) осознанно не устранялся перед
этим деплоем — решение принято оставить как есть для текущего масштаба
(доверенная группа из 10 человек). Вход через VK ID (Шаг 25) даёт
серверную сессию, но сами проверки на этих эндпоинтах — отдельный,
ещё не сделанный Шаг 26.
