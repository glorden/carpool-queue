...
| `POST` | `/orders/{order_id}/respond` | принять/отклонить предложенный заказ (требует сессию; `403`, если заказ предложен не вам) |
| `POST` | `/orders/{order_id}/self-assign` | самоназначиться на `pending`-заказ вне очереди (требует сессию, обязательная `reason`) — сразу становится исполнителем и уходит в конец очереди |
| `POST` | `/orders/{order_id}/complete` | отметить заказ завершённым (требует сессию; заказ должен быть в статусе `assigned` и назначен вам, иначе `403`) |
| `POST` | `/orders/{order_id}/cancel` | отменить заказ (требует сессию; из `pending`/`assigned`); если был `assigned` — водитель возвращается в начало очереди. `pending` может отменить только создатель, `assigned` — создатель или исполнитель, иначе `403` |
