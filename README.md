...
Добавляет пользователя и ставит его в конец **обеих** очередей (дальней и короткой).
...
| `GET` | `/queue` | список пользователей в текущем порядке очереди; query-параметр `queue_type` (`long`/`short`) — без него отдаёт обе очереди сразу |
| `POST` | `/orders` | создать новый заказ (`queue_type` обязателен — `long`/`short`; `route`, `comment` — опциональны) |
...
| `GET` | `/orders` | история заказов; query-параметры `status`, `user_id`, `queue_type`, `limit` (по умолчанию 100, максимум 500); сортировка по `created_at` (новые сверху) |
...
| `GET` | `/me` | текущий пользователь из сессии, либо `{"authenticated": false}` |
...
`/orders/{id}/respond`, `/orders/{id}/self-assign`, `/orders/{id}/complete`, `/orders/{id}/cancel` и `POST/PUT/DELETE /price` пока не проверяют identity вызывающего — любой `user_id` в теле запроса может действовать от чужого имени. Вход через VK ID уже даёт серверную сессию (см. выше), но сами проверки на этих эндпоинтах — отдельный, ещё не сделанный шаг. Это осознанное упрощение для доверенной группы из 10 человек, а не недосмотр. План устранения и обоснование — в [ARCHITECTURE.md](./ARCHITECTURE.md).
