...
- `static/js/nav.js` регистрирует service worker (`static/sw.js`, см.
  ...
  отдельно в каждом HTML-файле. При изменении `style.css`/любого файла
  под `static/js/`/`static/fav/` — бампнуть `CACHE_NAME` в `static/sw.js`,
  иначе у вернувшихся пользователей может надолго остаться старая версия.
